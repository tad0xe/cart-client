<template>
  <div class="container">
    <div class="columns">
      ...
      <div v-for="product in products.slice(0, 1)" :key="product._id">
      <div v-model="  totalPrice"  >{{product.price}}</div>
     </div>
    </div>
    <div class="columns">
      ...
      <div class="column is-12">
        ...
        >
          ...
          <span v-else>Place Order  ${{ getCartTotalPriceWithShipping }}</span>
        </button>
  ...
  methods: {
    ...
    stripeResponseHandler(status, response) {
      ...
      if (response.error) {
        ...
        // Send to our server
        axios.post(`https://taad.herokuapp.com/charge`, request).then(res => {
